...
const { checkNewOrder } = require('../middleware');

const ordersRouter = express.Router();
// 7.1. POST/api/orders - sukurti užsakymą su user_id, shop_item_id, quantity, total_price, status

ordersRouter.post('/api/orders', checkNewOrder, async (req, res) => {
  const {
...
ordersRouter.get('/api/orders', async (req, res) => {
  const sql = 'SELECT users.user_name, shop_items.shop_item_name, shop_items.shop_item_price FROM users JOIN orders ON users.user_id = orders.user_id JOIN shop_items ON shop_items.shop_item_id = orders.shop_item_id';
...
ordersRouter.get('/api/orders/user/:user_id', async (req, res) => {
  ...
  const sql = 'SELECT users.user_name, shop_items.shop_item_name, shop_items.shop_item_price FROM orders JOIN users ON users.user_id = orders.user_id JOIN shop_items ON shop_items.shop_item_id = orders.shop_item_id WHERE orders.user_id=?';
